...
TOKEN = "eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzUxMiJ9.eyJpc3MiOiJ2aXBjb21tZXJjZSIsImF1ZCI6ImFwaS1hZG1pbiIsInN1YiI6IjZiYzQ4NjdlLWRjYTktMTFlOS04NzQyLTAyMGQ3OTM1OWNhMCIsInZpcGNvbW1lcmNlQ2xpZW50ZUlkIjpudWxsLCJpYXQiOjE3NTE5MjQ5MjgsInZlciI6MSwiY2xpZW50IjpudWxsLCJvcGVyYXRvciI6bnVsbCwib3JnIjoiMTYxIn0.yDCjqkeJv7D3wJ0T_fu3AaKlX9s5PQYXD19cESWpH-j3F_Is-Zb-bDdUvduwoI_RkEeqbYCuxN0ppQQXb1ArVg"
ORG_ID = "161"
HEADERS_SHIBATA = {
...
def processar_item(item):
    ...
    for nagumo_url in nagumo_urls:
        ...
        if sku and sku.isdigit():
            ...
            if produto and (produto.get('stock', 0) > 0 or produto.get('stock') is None):
                ...
                # *** AJUSTE: Armazena a imagem no produto para a lógica de seleção de imagem ***
    ...
    # --- *** LÓGICA DE PRIORIDADE DE IMAGEM (AJUSTADA) *** ---
    # Prioriza a imagem do mercado com o melhor preço.
    
    if is_shibata_melhor:
        preco_principal_str = preco_shibata_str
        # Tenta pegar a imagem do melhor (Shibata)
        ...
        # Fallback: Se o melhor (Shibata) não tiver, tenta a do Nagumo (se existir)
        elif produtos_nagumo_ordenados and produtos_nagumo_ordenados[0].get('imagem_url'):
            imagem_principal = produtos_nagumo_ordenados[0]['imagem_url']

    elif is_nagumo_melhor:
        preco_principal_str = preco_nagumo_str
        # Tenta pegar a imagem do melhor (Nagumo)
        if produtos_nagumo_ordenados and produtos_nagumo_ordenados[0].get('imagem_url'):
            imagem_principal = produtos_nagumo_ordenados[0]['imagem_url']
        # Fallback: Se o melhor (Nagumo) não tiver, tenta a do Shibata (se existir)
        elif produtos_shibata_ordenados and produtos_shibata_ordenados[0].get('imagem_url'):
            imagem_principal = produtos_shibata_ordenados[0]['imagem_url']
        
    else:
        # Nenhum é melhor (ambos N/D)
        # Fallback: Pega a primeira imagem que encontrar (Prioriza Shibata)
        if produtos_shibata_ordenados and produtos_shibata_ordenados[0].get('imagem_url'):
            imagem_principal = produtos_shibata_ordenados[0]['imagem_url']
            preco_principal_str = preco_shibata_str # (Ajuste da lógica de fallback de preço)
        elif produtos_nagumo_ordenados and produtos_nagumo_ordenados[0].get('imagem_url'):
            imagem_principal = produtos_nagumo_ordenados[0]['imagem_url']
            preco_principal_str = preco_nagumo_str # (Ajuste da lógica de fallback de preço)
        
        # Se nenhum tiver imagem, 'imagem_principal' permanece DEFAULT_IMAGE_URL
        # Se nenhum tiver preço, 'preco_principal_str' permanece "N/D" (ou o fallback acima)
    ...
    resultado = {
        ...
        # Guarda a URL do PRODUTO COM O MELHOR PREÇO (AJUSTADO)
...
if resultados_comparacao:
    ...
    for item in resultados_filtrados:
        ...
        # URL da Imagem (Agora vem da lógica ajustada)
