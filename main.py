...
# Funções de Busca (mantida a de busca por página, embora não seja mais a principal)
def buscar_pagina_shibata(termo, pagina):
    """(FUNÇÃO DE FALLBACK) Busca produtos por termo, caso o ID não seja encontrado."""
...
# ----------------------------------------------------------------------
# NOVA FUNÇÃO DE BUSCA SHIBATA POR ID
# ----------------------------------------------------------------------
...
def obter_melhor_preco_shibata(produtos_ordenados):
    ...
    if 'preco_unidade_val' in melhor_produto and melhor_produto['preco_unidade_val'] != float('inf') and melhor_produto['preco_unidade_val'] > 0:
        ...
        match = re.search(r"/([a-zA-Z]+)", melhor_produto.get('preco_unidade_str', ''))
...
def realizar_comparacao_automatica():
    ...
    for item in lista_itens:
        ...
        # ----------------------------------------------------------------------
        # 1. Busca e Processamento Shibata (MODIFICADO)
        # ----------------------------------------------------------------------
        ...
        if match_shibata_id:
            ...
            # Aplica o mesmo processamento que era feito antes, mas para este item único
            if p and p.get("disponivel", True):
                preco = float(p.get('preco') or 0)
                em_oferta = p.get('em_oferta', False)
                
                # A API de detalhes pode ter uma estrutura de oferta diferente
                preco_oferta = p.get('preco_oferta') # Tentativa direta
                if not preco_oferta:
                ...
                # --- Lógica de cálculo de preço unitário (idêntica) ---
             ...
             st.warning(f"Não foi possível extrair ID do Shibata da URL: {shibata_url}. Item será ignorado.")

        # A lista terá 0 ou 1 item. A ordenação não afeta, mas é inofensiva.
        produtos_shibata_ordenados = sorted(produtos_shibata_processados, key=lambda x: x['preco_unidade_val'])

        # ----------------------------------------------------------------------
        # 2. Busca e Processamento Nagumo (SEM ALTERAÇÕES)
        # ----------------------------------------------------------------------
        ...
        # ----------------------------------------------------------------------
        # 3. Formata os Resultados Finais (SEM ALTERAÇÕES)
        # ----------------------------------------------------------------------
        ...
        resultados_finais.append({
            ...
            "shibata_preco_str": preco_shibata_str, # Adiciona para depuração/exibição
            "nagumo_preco_str": preco_nagumo_str   # Adiciona para depuração/exibição
        })
